...
void ScenariosList::createScenarios(TiXmlElement *xmlListOfScenarios,
                                    const std::string opDir) {
  ...
  if (finalYear <= 0 || finalYear <= baseYear) {
    std::cout << "Error in ScenariosList::createScenarios. The scenario list "
                 "start and end years are not properly defined."
              << std::endl;
    exit(1);
  }
  ...
  for (xmlScenario = xmlListOfScenarios->FirstChildElement("scenario");
       xmlScenario != NULL;
       xmlScenario = xmlScenario->NextSiblingElement("scenario")) {
    ...
    if (scenarios.empty() && sMonth > 0) {
      std::cout << "Error in ScenariosList::createScenarios. The first "
                   "scenario must begin at month zero."
                << std::endl;
      exit(1);
    } else if (sMonth < 0) {
      sMonth = 0;
    } else if (sMonth >= numMonths) {
      std::cout << "Error in ScenariosList::createScenarios. Scenario " << sname
                << " start month is beyond the end of the simulation."
                << std::endl;
      exit(1);
    }
    ...
    for (xmlOutput = xmlScenario->FirstChildElement("output");
         xmlOutput != NULL;
         xmlOutput = xmlOutput->NextSiblingElement("output")) {

      int t = -1;
      int minage = -1;
      if (!xmlOutput->Attribute("t", &t) || t < 0) {
        std::cout << "Error in ScenariosList::createScenarios. Scenario "
                  << sname << " output t is not defined." << std::endl;
        exit(1);
      } else if (t < sMonth) {
        std::cout << "Error in ScenariosList::createScenarios. Scenario "
                  << sname << " output t " << t
                  << " earlier than start of scenario." << std::endl;
        exit(1);
      }
      if (!xmlOutput->Attribute("minage", &minage))
        minage = -1; // default use values from params file for main output
                     // files

      outputEvents.push_back(
          PrevalenceEvent(t, minage, extraMinAge, extraMaxAge, outputMethod));
      // ignore any before start year
    }

    if (outputEvents.empty()) {
      std::cout << "Error in ScenariosList::createScenarios. Scenario " << sname
                << " defines no outputs." << std::endl;
      exit(1);
    }
    ...
    for (xmlMDA = xmlScenario->FirstChildElement("mda"); xmlMDA != NULL;
         xmlMDA = xmlMDA->NextSiblingElement("mda")) {
      ...
      if (!xmlMDA->Attribute("t", &t) || t < 0) {
        std::cout << "Error in ScenariosList::createScenarios. Scenario "
                  << sname << " mda t is not defined." << std::endl;
        exit(1);
      } else if (t < sMonth) {
        std::cout << "Error in ScenariosList::createScenarios. Scenario "
                  << sname << " mda t " << t
                  << " earlier than start of simulation." << std::endl;
        exit(1);
      }
      ...
      if (mdacov <= 0 || mdacov > 1.0) {
        std::cout << "Error in ScenariosList::createScenarios, scenario "
                  << sname << ". MDA coverage must be > 0 and <= 1.0"
                  << std::endl;
        exit(1);
      }
      if (sysComp < 0 || sysComp >= 1.0) {
        std::cout << "Error in ScenariosList::createScenarios , scenario "
                  << sname
                  << ". Systematic compliance for MDA must be >= 0 and < 1"
                  << std::endl;
        exit(1);
      }
      ...
      if (mdaType.empty()) {
        std::cout << "Error in ScenariosList::createScenarios, scenario "
                  << sname << ". MDA type is not defined" << std::endl;
        exit(1);
      }
    ...
    for (xmlBedNet = xmlScenario->FirstChildElement("bednet");
         xmlBedNet != NULL;
         xmlBedNet = xmlBedNet->NextSiblingElement("bednet")) {
      ...
      if (!xmlBedNet->Attribute("t", &t) || t < 0) {
        std::cout << "Error in ScenariosList::createScenarios. Scenario "
                  << sname << ". Bed net t is not defined." << std::endl;
        exit(1);
      } else if (t < sMonth) {
        std::cout << "Error in ScenariosList::createScenarios. Scenario "
                  << sname << " bed net month " << t
                  << " earlier than start of simulation." << std::endl;
        exit(1);
      }
      ...
      if (bednetCov < 0 || bednetCov > 1.0) {
        std::cout << "Error in ScenariosList::createScenarios, scenario "
                  << sname << ". Bed net coverage must be >= 0 and <= 1.0"
                  << std::endl;
        exit(1);
      }
      if (sysComp < 0 || sysComp >= 1.0) {
        std::cout << "Error in ScenariosList::createScenarios, scenario "
                  << sname
                  << ". Systematic compliance for bed nets must be >= 0 and < 1"
                  << std::endl;
        exit(1);
      }
    ...
    // prepend object representing zero bednets in case user didn't specify
    // value at month zero.
    ...
    for (xmlImp = xmlScenario->FirstChildElement("importation"); xmlImp != NULL;
         xmlImp = xmlImp->NextSiblingElement("importation")) {

      int t = -1;
      double aImp = -1;
      if (!xmlImp->Attribute("t", &t) || t < 0) {
        std::cout << "Error in ScenariosList::createScenarios. Scenario "
                  << sname << " importation t is not defined." << std::endl;
        exit(1);
      } else if (t < sMonth) {
        std::cout << "Error in ScenariosList::createScenarios. Scenario "
                  << sname << " importation t " << t
                  << " earlier than start of scenario." << std::endl;
        exit(1);
      }
    ...
    if (firstMonthaImp > 0)
      aImpEvents.insert(
          aImpEvents.begin(),
          ImportationRateEvent()); // default value of 1 for start of simulation

    // done, create scenario object
    scenarios.push_back(Scenario(numMonths, sMonth, sname, bedNets, mdaEvents,
                                 outputEvents, aImpEvents));

    if (_DEBUG)
      std::cout << "Scenario " << sname << " starts moth " << sMonth << ": "
                << mdaEvents.size() << " MDA events, Prevalence is output "
                << outputEvents.size() << " times." << std::endl;

  } // next scenario

  if (!scenarios.size()) {
    std::cout << "Error in ScenariosList::createScenarios. No scenarios found "
                 "in file."
              << std::endl;
    exit(1);
  }

  // Now calculate the months that we need to save as each scenario runs. This
  // is done as follows 1) For scenario n, examine the start month for scenarios
  // n+1 ... end 2) if their start month is both later than the start month for
  // scenario n AND earlier than any other month that scenario n will save THEN
  ...
  for (unsigned s = 0; s < scenarios.size(); s++) {
    ...
    for (unsigned t = s + 1; t < scenarios.size(); t++) {
      ...
      if (newStartMonth <= currentStartMonth)
        ...
        earliestMonthSaved = (earliestMonthSaved < newStartMonth)
                                 ? earliestMonthSaved
                                 : newStartMonth;
      }
    }
