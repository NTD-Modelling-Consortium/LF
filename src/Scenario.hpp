...
public:
    ...
    Scenario(int numMonths, int sMonth, std::string nm) : startMonth(sMonth), name(nm) {
    ...
    Scenario(int numMonths, int sMonth, std::string nm, std::vector<BedNetEvent>& bn, std::vector<MDAEvent>& mda, std::vector<PrevalenceEvent>& prevOut, std::vector<ImportationRateEvent>& aImpF) : startMonth(sMonth), name(nm) {
        ...
        std::sort(bedNets.begin(), bedNets.end(), BedNetEvent::sort); //overloaded <opertor does compare
        std::sort(mdaEvents.begin(), mdaEvents.end(), MDAEvent::sort);
        std::sort(outputEvents.begin(), outputEvents.end(), PrevalenceEvent::sort);
        std::sort(aImpFactors.begin(), aImpFactors.end(), ImportationRateEvent::sort);
        ...
        for (unsigned month = 0; month < outputEvents.size(); month++){
            ...
            if(prevMonth < numMonths)
                outputDue[prevMonth] = &(outputEvents[month]); //one entry fo revery month, null pointer if outpu tnot needed, otherwise pointer to a structure giving the details
            
        }
        
        //record age range of extea output for creating file names. These will be the same for all outputs so just use first
        minAgeExtra = outputEvents[0].getMinAgeExtra();
        maxAgeExtra = outputEvents[0].getMaxAgeExtra();
        requiresExtra = ((minAgeExtra > 0) || (maxAgeExtra < Population::getMaxAge()));
        ...
        if (!bedNetCoverage[0]){
            std::cout << "Error in Scenario::Scenario, scenario " << name << ". Bednet coverage not completely defined."<< std::endl;
            exit(1);
        }
        ...
        if (!aImpUpdate[0]){
            std::cout << "Error in Scenario::Scenario, scenario " << name << ". Importation rate not completely defined."<< std::endl;
            exit(1);
        }
...
*/
    ...
    void writeRoadmapTarget(Population& popln, int t, int rep, int DoMDA, int TAS_Pass, std::string folder);
