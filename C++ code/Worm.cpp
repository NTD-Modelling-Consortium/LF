...
Worm::Worm(TiXmlElement* xmlParameters){
    ...
    if (xmlWorm == NULL){
        std::cout << "Error in Worm::Worm. Cannot find worm parameters." << std::endl;
        exit(1);
    ...
    for ( xmlParam = xmlWorm->FirstChildElement("param"); xmlParam != NULL; xmlParam = xmlParam->NextSiblingElement("param")){
        ...
        if (name == "nu"){
            ...
            std::cout << "Unknown parameter " << name << " in Worm parameter list." << std::endl;
    }
    
    
    if(gotAll < 7){
        std::cout << "Error in Worm::Worm. File  does not contain all the required parameters."<< std::endl;
        exit(1);
    }else
        std::cout << "Worm parameters: nu=" << nu << ", alpha=" << alpha << ", psi1=" << psi1 << ", psi2=" << psi2 << ", s2=" << s2 << ", mu=" << mu << ", gamma=" << gamma << std::endl<< std::endl;
...
int Worm::wormsTreated(int W, std::string type) {
    ...
    if (type=="da"){
        ...
        propWormsKilled = (wPropMDA >=0)?wPropMDA:0.75;
        fecRed = 11;
    }else if (type=="ma2"){
