...
public:
    ...
    monthToOuput(): month(0), bedNetCov(0.0), bednetSysComp(-1.0), aImpFactor(1.0), prevalenceNeeded(false), minAgePrev(0), mdaNeeded(false), mdaCov(0.0), mdaType(std::string("-")), mdaSysComp(-1.0), minAgeMDA(0) {};
    monthToOuput(int mnth, double bnc, double bns, double aif):  month(mnth), bedNetCov(bnc), bednetSysComp(bns), aImpFactor(aif), prevalenceNeeded(false), minAgePrev(0), mdaNeeded(false), mdaCov(0.0), mdaType(std::string("-")), mdaSysComp(-1.0), minAgeMDA(0) {};
...
class Output {
    ...
    void saveMonth(int month, Population& popl, PrevalenceEvent* outputNeeded, RecordedPrevalence prevalence, MDAEvent* mda = NULL);
